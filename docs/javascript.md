...
- `date` 클래스는 자바스크립트에서 날짜와 시간으 다루기 위해 사용하는 타입이다.
- 객체 함수
  - `getFullYear()` : 호출 대상이 가지고 있는 일시의 연도를 반환한다.
  - `getMonth()` : 호출 대상이 가지고 있는 일시의 월을 `0` 이상 `11`이하의 숫자로 반환한다. 이때 `0` 을 1월이라고 한다.
  - `getDate()` : 호출 대상이 가지고 있는 일시의 일을 반환한다.
  - `getDay()` : 호출 대상이 가지고 있는 일시의 요일을 `0` 이상 `6` 이하의 숫자로 반환한다. 이때 `0` 을 일요일이라고 한다.
  - `getHours()` : 호출 대상이 가지고 있는 일사의 시를 24시제인 `0` 이상 `23` 이하의 숫자로 반환한다.
  - `getMinutes()` : 호출 대상이 가지고 있는 일시의 분을 숫자로 반환한다.
  - `getSeconds()` : 호출 대상이 가지고 있는 일시의 초을 숫자로 반환한다.
  - `getTime()` : 호출 대상이 가지고 있는 일시를 UNIX 타임인 숫자로 반환한다.


### localStorage

- `localStorage` 는 자바스크립트에서 로컬 브라우저에 저장되는 소규모의 데이터에 접근하기 위해 사용하는 `Storage` 타입의 객체이다.
- 객체 함수
  - `getItem(x)` : 키가 `x` 인 문자열을 반환한다. 그러한 키를 가지는 항목이 없다면 `null` 을 반환한다.
  - `setItem(k, v)` : 키가 `x`, 값이 문자열 `v` 인 쌍을 저장한다.
