...
- 정적 함수
  - `abs(x)` : 숫자인 `x` 의 절댓값을 반환한다. **사용**
  - `cbrt(x)` : 숫자인 `x` 의 세제곱근을 반환한다. **사용**
  - `sqrt(x)` : 숫자인 `x` 의 제곱근을 반환한다. **사용** 
  - `ceil(x)` : 숫자인 `x` 를 정수부까지 올림하여 반환한다. **사용**
  - `floor(x)` : 숫자인 `x` 를 정수부까지 내림하여 반환한다. **사용**
  - `round(x)` : 숫자인 `x` 를 정수부까지 반올림하여 반환한다. **사용**
  - `trunc(x)` : 숫자인 `x` 의 소수부를 버리고 반환한다. **사용**
  - `Math.max(...n)` : 숫자를 인자로 가지는 가변인자 `n` 중 가장 큰 값을 반환한다. **사용**
  - `Math.min(...n)` : 숫자를 인자로 가지는 가변인자 `n` 중 가장 작은 값을 반환한다. **사용**
