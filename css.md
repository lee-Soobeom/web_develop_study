...
- 가령, 모든 `<span>` 태그를 선택하기 위해 아래와 같이 선택자를 작성한다.
...
## gap
- `gap` 속성은 플렉스 컨테이너가 포함하는 아이템 간의 거리를 지정하기 위해 사용한다.
- 형태
    - `[크기]` : 행간, 열간 거리를 모두 지정한다.
    - `[크기: v] [크기: h]` : 행간 거리를 `v` 로, 열간 거리를 `h` 로 지정한다.
...
- 그리드 컨테이너가 될 요소의 `display` 속성값을 `grid` 혹은 `inline-grid`로 설정한다.

# 그리드 컨테이너(부모)
## grid-template-columns
- `grid-template-columns` 속성은 그리드 컨테이너에 부여하는 속성으로 그리드 열 구조를 설정하기 위한 속성이다. 값을 공백으로 구분하여 여러개 작성할 수 있다.
- 값
    - 크기 값
    - `fr` 값
- 형태
...
## gap
- `gap` 속성은 그리드 컨테이너가 ㅍ포함하는 아이템간의 거리를 지정하기 위해 사용한다.
- 형태
    - `[크기]` : 행간, 열간 거리를 모두 지정한다.
    - `[크기: v] [크기: h]` : 행간 거리를 `v`, 열간 거리를`h`
...
## order
- `order` 속성은 해당 아이템의 태그 순번과 관계없이, 시작적으로 배치되어야 하는 순서를 지정하기 위해 사용한다.
